...
\paragraph{}
Un cop hem arribat a una quantitat de triangles donada i l'error que causen els triangles està per sota d'un nombre donat ja s'ha acabat la creació de la malla i aquesta pasa a visualitzar-se. L'algorisme torna a entrar en lloc quan l'observador es mou. En aquest moment la distància de cada triangle respecte a l'observador canvia i, per tant, també ho fa l'error ponderat, això pot fet que grups de 4 triangles es fusionin en 2 triangles ja que la nova posició de l'observador fa que no calgui tanta presició en aquesta regió del terreny, o que es parteixin nous triangles per fer més exactes regions que amb la posició anterior ja eren prou bones.
